...
import {
  ...
  WalletProviderFactoryArgs,
  WalletProviderType,
  Wallet_Provider,
} from './walletProvider/factory';
...
export class Wallet {
  ...
  transferSol = async (destination: PublicKey, amount: bigint | number) => {
    //return nativeTransfer(this.connection, this, destination, amount);
  };
  closeTokenAccount = async (
    publicKey: PublicKey,
  ...
  createSignature = async (message: Buffer) => {
...
const WalletContext = React.createContext<WalletContextProps | undefined>(
  undefined
);
